...
PILOT_DATA_RAW_DIR = BASE_DIR / "Data" / "Raw"
FULL_DATA_RAW_DIR = BASE_DIR / "Data" / "Full" / "Raw"

PILOT_RESULTS_RAW_DIR = BASE_DIR / "Results" / "Raw"
FULL_RESULTS_RAW_DIR = BASE_DIR / "Results" / "Full" / "Raw"

FULL_GROUND_TRUTH_FILE = (
    FULL_DATA_RAW_DIR / "full_ground_truth_raw.csv"
)
...
def check_required_data_files():
    print("Checking Pilot data: Data/Raw...")
    print("-" * 60)
    ...
    for filename in REQUIRED_PILOT_DATA_FILES:
        path = PILOT_DATA_RAW_DIR / filename
    ...
    if missing_files:
        raise FileNotFoundError(
            "Missing required Raw Pilot data files: "
            + ", ".join(missing_files)
        )

    if empty_files:
        raise ValueError(
            "Empty required Raw Pilot data files: "
            + ", ".join(empty_files)
        )

    print("All required Raw Pilot data files exist.")
    print()

    print("Checking Full data: Data/Full/Raw...")
    print("-" * 60)
    ...
    for filename in REQUIRED_FULL_DATA_FILES:
        path = FULL_DATA_RAW_DIR / filename
    ...
    if missing_files:
        raise FileNotFoundError(
            "Missing required Raw Full data files: "
            + ", ".join(missing_files)
        )

    if empty_files:
        raise ValueError(
            "Empty required Raw Full data files: "
            + ", ".join(empty_files)
        )

    print("All required Raw Full data files exist.")
    print()


def check_results_status():
    print("Checking Raw result folders...")
    print("=" * 60)

    print("Pilot results: Results/Raw")
    print("-" * 60)

    if not PILOT_RESULTS_RAW_DIR.exists():
        print("PENDING : Results/Raw folder does not exist yet.")
        print("This is OK before running the Raw pilot experiment.")
    else:
        readme_file = PILOT_RESULTS_RAW_DIR / "README.md"
        gitkeep_file = PILOT_RESULTS_RAW_DIR / ".gitkeep"
        ...
        for filename in EXPECTED_PILOT_RESULT_FILES:
            path = PILOT_RESULTS_RAW_DIR / filename
    ...
    print()
    print("Full results: Results/Full/Raw")
    print("-" * 60)

    if not FULL_RESULTS_RAW_DIR.exists():
        print("PENDING : Results/Full/Raw folder does not exist yet.")
        print("This is OK before running the Raw full experiment.")
    else:
        for filename in EXPECTED_FULL_RESULT_FILES:
            path = FULL_RESULTS_RAW_DIR / filename
    ...
    print("=" * 60)
    print("Raw result files may be pending before running the LLM.")
    print()
...
def main():
    ...
    pilot_sample_rows = read_csv_rows(PILOT_DATA_RAW_DIR / "pilot_sample_raw.csv")
    pilot_gt_rows = read_csv_rows(PILOT_DATA_RAW_DIR / "pilot_ground_truth_raw.csv")
    full_sample_rows = read_csv_rows(FULL_DATA_RAW_DIR / "full_sample_raw.csv")
    full_gt_rows = read_csv_rows(FULL_GROUND_TRUTH_FILE)
    author1_rows = read_csv_rows(PILOT_DATA_RAW_DIR / "pilot_annotation_raw_author1.csv")
    author2_rows = read_csv_rows(PILOT_DATA_RAW_DIR / "pilot_annotation_raw_author2.csv")
    kappa_rows = read_csv_rows(PILOT_DATA_RAW_DIR / "kappa_scores_raw.csv")
    ...
    print(f"full_sample_raw.csv               : {len(full_sample_keys)} cases")
    ...
    print()
    print("Raw folder check completed successfully.")
